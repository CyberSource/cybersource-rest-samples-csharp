...
namespace Cybersource_rest_samples_dotnet
{
    ...
    {
        private static readonly string PathOfSamplesFolder = Path.Combine($"{AppDomain.CurrentDomain.BaseDirectory}", $"../../../Source/Samples");
        ...
        private static void ShowMethods()
        {
            ...
            foreach (var apiFamily in apiFamilies)
            {
                ...
                foreach (var api in ApiList.Where(api => api.ApiFamily == apiFamily).OrderBy(api => api.ApiFunctionCall))
                {
                    ...
                    lineCharsCount += api.ApiFunctionCall.Length;
                }
        ...
        private static void InitializeApiList()
        {
            ...
            foreach (var apiFamily in apiFamilies)
            {
                ...
                {
                    var lastForwardSlashIndex = file.LastIndexOf(@"/", StringComparison.Ordinal);
                    var lastBackSlashIndex = file.LastIndexOf(@"\", StringComparison.Ordinal);

                    if (lastBackSlashIndex < lastForwardSlashIndex)
                    {
                        lastBackSlashIndex = lastForwardSlashIndex;
                    }
